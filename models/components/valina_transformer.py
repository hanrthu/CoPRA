...
from models.components.valina_attn import MultiHeadSelfAttention, FlashMultiHeadSelfAttention
    ...
    def forward(self, x, key_padding_mask=None, need_attn_weights=False):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if torch.isnan(x).any():
                print("Find Nan in X! at layer {}".format(i), x.shape)
            if need_attn_weights:
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, input, key_padding_mask=None, need_attn_weights=None):
