...
def _index_select_with_dist(v, index, n):
    if isinstance(v, torch.Tensor) and v.size(0) == n and (len(v.shape) == 1 or v.size(0) != v.size(1)):
        return v[index]
    elif isinstance(v, torch.Tensor) and v.size(0) == v.size(1) and v.size(0) == n:
        tmp = v[index]
        new_v = tmp[:, index]
        return new_v
    elif isinstance(v, list) and len(v) == n:
...
def _index_select_complex(data, index):
    return {
        k: _index_select_with_dist(v, index, data['restype'].size(0))
        for k, v in data.items()
    }
